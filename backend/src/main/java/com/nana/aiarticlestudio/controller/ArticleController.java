...
public class ArticleController {
    ...
    public BaseResponse<ArticleVO> continueWorkflow(@PathVariable String taskId) {
        return BaseResponse.success(articleService.continueWorkflow(taskId));
    }

    @PostMapping("/generate-image-results/{taskId}")
    public BaseResponse<ArticleVO> generateImageResults(@PathVariable String taskId,
                                                        @RequestParam(defaultValue = "AUTO") String provider) {
        return BaseResponse.success(articleService.generateImageResults(taskId, provider));
    }
    ...
    public BaseResponse<ArticleVO> composeArticle(
            @PathVariable String taskId
    ) {
        return BaseResponse.success(
                articleService.composeArticle(
                        taskId
                )
        );
    }
